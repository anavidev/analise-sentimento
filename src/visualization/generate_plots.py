...
from utils.utils import pd, valor_barras, grafico_colunas_1, grafico_barras

def gerar_graficos(resultados_rb, resultados_mlb):
    
    # comparacao de acuracia e f1-score entre rule-based e ml-based
    acuracia_rb = resultados_rb["acuracia"]
    f1_score_rb = resultados_rb["f1_score"]
    qtd_comentarios_antes_rb = resultados_rb["qtd_comentarios_antes"]
    qtd_comentarios_depois_rb = resultados_rb["qtd_comentarios_depois"]

    acuracia_mlb = resultados_mlb["acuracia"]
    f1_score_mlb = resultados_mlb["f1_score"]
    qtd_comentarios_antes_mlb = resultados_mlb["qtd_comentarios_antes"]
    qtd_comentarios_depois_mlb = resultados_mlb["qtd_comentarios_depois"]

    dados_acuracia = pd.DataFrame({
        'Algoritmo': ['Rule-Based', 'Machine Learning Based'],
    ...
    plt.suptitle("-- 25% de todos os dados foram utilizados para teste --\n-- Os dados de teste estão desbalanceados --")
    ...
    sentimentos_mlb.set_xlim(right=7000)
